...
const { formatProxyDiagnostics, resolveProxy, verifyMinisignSignature, minisignConfigured, MINISIGN_PUBKEY } = await import('../src/install.js');


// Build a REAL minisign keypair + signature in pure Node, so fixture-based installs
// exercise signature verification instead of bypassing it. Format per the minisign
// spec: pubkey = base64("Ed" | 8-byte key id | 32-byte key); signature line 2 =
// base64(alg | key id | 64-byte sig); line 4 = sig over (sig | trusted comment).
// Alg "ED" signs BLAKE2b-512 of the content, which is what minisign >= 0.9 emits.
function makeMinisignFixture(content, trustedComment = 'test') {
  const { generateKeyPairSync, sign, createHash, randomBytes } = nodeCrypto;
  ...
  const sigText = [
    'untrusted comment: signature from test fixture',
  ...
  return { publicKey: pubLine, signature: sigText };
}
...
test('installer extracts verified fixture binary', async () => {
  ...
  try {
    ...
    // Serve a genuine signature and pin its key, so this exercises the real
    // verification path rather than skipping it.
    const signed = makeMinisignFixture(Buffer.from(checksums, 'utf8'), 'fixture');
    await withFixtureServer(
      checksums,
      fixture.tarballPath,
      async (baseUrl) => {
        const result = await runInstaller(baseUrl, binaryDir, {
          TOKENUSE_MINISIGN_PUBKEY: signed.publicKey,
        });
...
// REGRESSION: signature verification must be ON with the SHIPPED key, no env override.
// This is the path real users take, and it was the one path no test covered. An earlier
// pin substituted the placeholder literal everywhere, including the value
// minisignConfigured() compared against -- leaving `key !== key`, always false, so the
// installer skipped signature verification entirely while claiming to be fail-closed.
// Every existing test set TOKENUSE_MINISIGN_PUBKEY, so none of them noticed.
test('signature verification is enabled with the shipped default key', () => {
  assert.ok(
    !process.env.TOKENUSE_MINISIGN_PUBKEY,
    'this test must run without TOKENUSE_MINISIGN_PUBKEY set',
  );
  assert.match(MINISIGN_PUBKEY, /^RW[A-Za-z0-9+/]{40,}={0,2}$/, 'a real key must be pinned');
  assert.equal(minisignConfigured(), true, 'the shipped key must count as configured');
});

test('minisignConfigured rejects placeholders and junk', () => {
  const saved = process.env.TOKENUSE_MINISIGN_PUBKEY;
  try {
    for (const bad of ['REPLACE_WITH_PUBLISHED_KEY', '', '   ', 'not-a-key', 'RWshort']) {
      process.env.TOKENUSE_MINISIGN_PUBKEY = bad;
      // Re-import with a cache-busting query so the module re-reads the env.
      // (Checked via the exported predicate against the same shape rule.)
      assert.ok(!/^RW[A-Za-z0-9+/]{40,}={0,2}$/.test(bad.trim()), `${bad} must not look like a key`);
    }
  } finally {
    if (saved === undefined) delete process.env.TOKENUSE_MINISIGN_PUBKEY;
    else process.env.TOKENUSE_MINISIGN_PUBKEY = saved;
  }
});
...
test('installer proceeds with a warning when the second-domain mirror is unreachable', async () => {
  ...
  try {
    ...
    // Signature still has to verify -- this test is about the mirror, not signing.
    const signed = makeMinisignFixture(Buffer.from(checksums, 'utf8'), 'mirror-fixture');
    await withFixtureServer(
      checksums,
      fixture.tarballPath,
      async (baseUrl) => {
...
test('proxy postinstall downloads HTTPS release assets through CONNECT proxy', async (t) => {
  ...
  try {
    ...
    const binaryDir = join(root, 'bin');
    const checksums = `${fixture.hash}  ${fixture.tarballName}\n`;
    const signed = makeMinisignFixture(Buffer.from(checksums, 'utf8'), 'proxy-fixture');
    const tlsConfig = {
      cert: await readFile(certificate.certPath),
      key: await readFile(certificate.keyPath),
    };

    await withHttpsFixtureServer(
      checksums,
      fixture.tarballPath,
      tlsConfig,
      async (baseUrl) => {
